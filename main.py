...
class AttendanceApp(ctk.CTk):
    ...
    def load_encodings(self):
        """Loads face encodings from pickle file if it exists."""
        if os.path.exists(ENCODINGS_FILE):
            ...
            self.update_status("No database found. Register users.")

    def update_status(self, text):
        self.status_label.configure(text=f"Status: {text}")

    def register_face(self):
        """Captures a single frame, detects a face, and saves the encoding."""
        ...
        # Open temp camera for registration
        ...
        # Encode face (Get the first face found)
    ...
    def mark_attendance(self, name):
        ...
        # Read existing attendance to avoid duplicate entries for the same day
        if file_exists:
...
if __name__ == "__main__":
    import numpy as np # Importing here to ensure availability
    app = AttendanceApp()
    app.mainloop()